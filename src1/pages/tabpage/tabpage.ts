...
import { IonicPage, NavController, NavParams, Events, App, AlertController, Searchbar, ActionSheetController, ToastController, Platform, Alert } from 'ionic-angular';
...
export class TabpagePage {
    ...
    constructor(public navCtrl: NavController, public navParams: NavParams, public events: Events, public provider: AuthproviderProvider, public app: App, public alertCtrl: AlertController, public storage: Storage, public actionSheetCtrl: ActionSheetController, public camera: Camera, public platform: Platform, public toastCtrl: ToastController, public file: File, public filePath: FilePath, public googlePlus: GooglePlus, public firebase: Firebase, public socket: Socket, public localNotifications: LocalNotifications, public geolocation: Geolocation) {
    ...
    onInput() {
        ...
        if (this.myInput.toString().length > 1) {
            this.searchdataarray = this.searchdataarray.filter((item) => {
                return item.PilotFname.toLowerCase().indexOf(this.myInput.toLowerCase()) > -1 || item.PilotLname.toLowerCase().indexOf(this.myInput.toLowerCase()) > -1;
            });
    ...
    geocodeLatLng(geocoder, latlng) {
        ...
        geocoder.geocode({ 'location': latlng1 }, function (results, status) {
            ...
            if (status === 'OK') {
                ...
                var countryname = results[8].formatted_address.toString();
