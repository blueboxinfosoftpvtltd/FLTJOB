...
export class AirportcodePage {
  ...
  ionViewDidEnter() {
    this.navBar.backButtonClick = () => {
      ...
      this.enrouteairportcodelist3 = localStorage.getItem("enrouteairportcodelist");
      if(this.enrouteairportcodelist3 != null){
        this.enrouteairportcodelist  = JSON.parse(this.enrouteairportcodelist3);
        //for remove duplicat element from array
        var uniq = {}
        this.enrouteairportcodelist =  this.enrouteairportcodelist.filter(obj => !uniq[obj.pkAirportcodeId] && (uniq[obj.pkAirportcodeId] = true));
      }


      if (this.enrouteairportcodelist !== undefined) {
        for (let i = 0; i < this.enrouteairportcodelist.length; i++) {
          if (this.enrouteairportcodelist[i].checked == true) {
            this.selectedaiportcode.push(this.enrouteairportcodelist[i].AirportCode);
