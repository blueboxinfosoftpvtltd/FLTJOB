...
export class FiltercaptainPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private selector: WheelSelector, public provider: AuthproviderProvider, public events: Events, public alertCtrl: AlertController, public storage: Storage, public modalCtrl: ModalController, public sprovider: SharedserviceProvider) {
    ...
    this.provider.getContinents().subscribe(res => {
      ...
      if (this.gdata) {
        this.selectedociens = this.gdata.Ocean;
    ...
    this.provider.getContinents().subscribe(res => {
      ...
      if (this.gdata) {
        ...
        this.sprovider.getcontinentlist(this.continentlist, this.continents);
      }
    });
    this.title = 'Required Experience - Captain';
    ...
    console.log("isgender " + this.isgender);
  ...
  ionViewDidEnter() {
    ...
    if (this.gdata) {
      ...
      this.pictime = this.gdata.AircraftPICtime;
      this.pictimetype = this.gdata.PICTimeAircraftId;
      this.mclass = this.gdata.FAAMedical;
      this.ratingcount = this.gdata.RatingCount;

      if (this.gdata.HavePassport == true) {
        this.validpassyes = true;
      }
      else {
        this.validpassno = true;
      }
      if (this.gdata.rating == "ATP") {
        this.ratinga = true;
      }
      else {
        this.ratingc = true;
      ...
      if (this.gdata.IsReqPrev12MonthTraining == true) {
  ...
  next() {
    if (this.scommand == true) {
      ...
      this.navCtrl.push('FilterflightattendantPage', { 'id': this.id, 'tripid': this.tripid, 'aircraftname': this.aircraftname, 'fatt': this.fatt, 'fins': this.fins, 'isprofile': this.isprofile, 'isgender': this.isgender });
    }
    else if (this.fins == true) {
      this.navCtrl.push('FilterflightinstructorPage', { 'id': this.id, 'tripid': this.tripid, 'aircraftname': this.aircraftname, 'fatt': this.fatt, 'fins': this.fins, 'isprofile': this.isprofile, 'isgender': this.isgender });
    }
  }
